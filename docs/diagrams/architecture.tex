...
% COLOR PALETTE
% =============================================================================
\definecolor{pretrainedgreen}{RGB}{120, 190, 120}   % Pretrained (frozen)
\definecolor{learnedblue}{RGB}{130, 170, 220}        % Learned (trainable)
...
\tikzset{
    ...
    pretrainedbox/.style={base, fill=pretrainedgreen},
    learnedbox/.style={base, fill=learnedblue},
    functionbox/.style={base, fill=functionorange},
    tokenframe/.style={
        draw=arrowblack, fill=learnedblue!35, rounded corners=10pt,
        minimum width=4.9cm, minimum height=2.45cm,
        line width=0.7pt
    },
    tokenpill/.style={
        draw=arrowblack!55, fill=learnedblue!20,
        minimum width=0.44cm, minimum height=0.30cm,
...
\begin{tikzpicture}
    ...
    \node[inputbox, minimum height=2.8cm] (images) at (5.45, 0.5)
        {Observations\\[2pt]\includegraphics[width=2.1cm]{obs.png}\\[2pt]$\mathbf{o}_{t}$};
    ...
    % --- Backbone and output ---
    \node[tokenframe]    (tokenframe) at (18, 3.75) {};
    \foreach \dx in {-1.38, -0.92, -0.46, 0.00, 0.46, 0.92, 1.38} {
        \node[tokenpill] at ($(tokenframe.center)+(\dx,-0.75)$) {};
    }
    \node[backbonebox]   (gemma)  at (18, 4.10)  {Gemma-3 270M};
    \node[functionbox]   (pool)   at (22.5, 4.10){Pool\\Last Token};
    \node[learnedbox]    (vhead)  at (26.5, 4.10){GELU MLP\\Value Head};
    \node[outputbox, inner sep=5pt, minimum height=0cm, minimum width=0cm] (bins) at (30.5, 4.10)
        {\includegraphics[width=2cm]{distribution.png}\\[-4pt]{\tiny Monte Carlo Distribution}};
    \coordinate (visiontokenin) at ($(tokenframe.south)+(-0.95, 0)$);
    \coordinate (texttokenin) at ($(tokenframe.south)+(0.95, 0)$);
    ...
    \begin{scope}[on background layer]
        ...
        \node[draw=arrowblack!30, rounded corners=5pt, fill=groupgray!50,
              fit=(tokenframe)(gemma)(pool)(vhead)(bins), inner sep=12pt,
    ...
    \node[fill=pretrainedgreen, draw=arrowblack, minimum width=0.5cm, minimum height=0.5cm,
    ...
    \node[fill=learnedblue, draw=arrowblack, minimum width=0.5cm, minimum height=0.5cm,
